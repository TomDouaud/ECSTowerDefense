...
use crate::{
    ...
    enemy::{Enemy, Health, HealthBar}, // On réutilise les composants Health/HealthBar
...
fn update_sim_visuals(
    ...
    // On récupère Visibility pour pouvoir le changer
    mut bars: Query<(&mut Transform, &Parent, &mut Visibility), (With<HealthBar>, Without<Enemy>)>,
    // On récupère aussi le fond noir (c'est le frère de la barre rouge)
    // Pour simplifier, ici on ne cache que la barre rouge, le noir restera invisible si on ne touche pas à son visibility
    // OU on fait une query plus complexe.
    // Solution simple : On suppose que HealthBar gère la visibilité.
