...
fn setup_game(
    ...
    sim_entities: Query<Entity, With<crate::simulation::SimComponent>>, // Import nécessaire !
) {
...
fn grid_click_interaction(mut commands: Commands, mouse: Res<ButtonInput<MouseButton>>, win: Query<&Window, With<PrimaryWindow>>, cam: Query<(&Camera, &GlobalTransform)>, sel: Res<SelectedTower>, ass: Res<GameAssets>, mut stats: ResMut<PlayerStats>, pause: Res<GlobalPause>) {
    ...
    // ... (Reste de la logique de clic inchangée) ...
    // Copiez votre logique de l'étape précédente ici si besoin, ou demandez-moi
...
fn cleanup_game(
    mut cmd: Commands, 
    // On récupère TOUT ce qui bouge ou qui tire
